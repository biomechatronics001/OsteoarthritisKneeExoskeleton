...
CIC::~CIC()
{}
void CIC::CIC_update(double TAV, double MotorAnlge_L, double MotorAnlge_R, double MotorVelocity_L, double MotorVelocity_R)
{
  qm_L=MotorAnlge_L;
...
void CIC::pos_vel_estimate(double TAV)
{
...
void CIC::gait_estimation()
{
  ...
  GP_IMU_L = (atan2(-dqTd_L_nor, qTd_L_nor) * 180 / PI) - 90;
  if (GP_IMU_L < 0)
  {
  ...
  GP_IMU_R = (atan2(-dqTd_R_nor, qTd_R_nor) * 180 / PI) - 90;
  if (GP_IMU_R < 0)
  {
...
//  T_command_R=(kd_R*(qhr_R-(qm_R/n))+bd_R*(dqhr_R))*k_col_imp_R+Jm*ddqhr_R+bm*dqhr_R; //-dqme_R/n
 ...
 T_command_R=(kd_R_Junxi*(qhr_R-(qm_R/n))+bd_R_Junxi*(dqhr_R))*k_col_imp_R+Jm*ddqhr_R+bm*dqhr_R; //-dqme_R/n
  ...
  impedanceTorque_R = CICGain_L*T_command_R;
...
void CIC::set_P_ahead(double leading)
{
  P_ahead = leading;
}

void CIC::calculateSigmoid(double maxValue,double minValue, double *sigmoid_array){
...
void CIC::computeGaitPhase(double TAV, double MotorAnlge_L, double MotorAnlge_R, double MotorVelocity_L, double MotorVelocity_R)
{
  qm_L=MotorAnlge_L;
  dqm_L=MotorVelocity_L;   
  qm_R=MotorAnlge_R;
...
void CIC::addSineAssistanceProfile(int sineAssistanceTypeIdx, double sineAssistanceMagnitude, int sineAssistanceShift, int sineAssistanceDuration, double sineAssistanceSaturation)
{
  if (sineAssistanceTypeIdx == 1) // Flexion
  {
    ...
    for (int i = 0; i < sineAssistanceDuration+1; i++)
    {
      ...
      {
        sineAssistanceFlexionProfileList[sineAssistanceFlexionProfileCounter][sineAssistanceShift+i] = -min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
      else
      {
        sineAssistanceFlexionProfileList[sineAssistanceFlexionProfileCounter][sineAssistanceShift+i-100] = -min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
    }
  ...
  else if (sineAssistanceTypeIdx == 2) // Extension
  {
    ...
    for (int i = 0; i < sineAssistanceDuration+1; i++)
    {
      if (sineAssistanceShift+i < 100)
      {
        sineAssistanceExtensionProfileList[sineAssistanceExtensionProfileCounter][sineAssistanceShift+i] = min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
      else
      {
        sineAssistanceExtensionProfileList[sineAssistanceExtensionProfileCounter][sineAssistanceShift+i-100] = min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
    }
...
void CIC::removeSineAssistanceProfile(int sineAssistanceTypeIdx, int sineAssistanceProfileIdx)
{
...
void CIC::editSineAssistanceProfile(int sineAssistanceTypeIdx, int sineAssistanceProfileIdx, double sineAssistanceMagnitude, int sineAssistanceShift, int sineAssistanceDuration, double sineAssistanceSaturation)
{
  if (sineAssistanceTypeIdx == 1) // Flexion
  {
    ...
    for (int i = 0; i < sineAssistanceDuration+1; i++)
    {
      if (sineAssistanceShift+i < 100)
      {
        sineAssistanceFlexionProfileList[sineAssistanceProfileIdx][sineAssistanceShift+i] = -min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
      else
      {
        sineAssistanceFlexionProfileList[sineAssistanceProfileIdx][sineAssistanceShift+i-100] = -min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
    }
  }
  else if (sineAssistanceTypeIdx == 2) // Extension
  {
    ...
    for (int i = 0; i < sineAssistanceDuration+1; i++)
    {
      if (sineAssistanceShift+i < 100)
      {
        sineAssistanceExtensionProfileList[sineAssistanceProfileIdx][sineAssistanceShift+i] = min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
      else
      {
        sineAssistanceExtensionProfileList[sineAssistanceProfileIdx][sineAssistanceShift+i-100] = min(sineAssistanceMagnitude / gearRatio * sin(3.1415926 / sineAssistanceDuration * i), sineAssistanceSaturation / gearRatio);
      }
    }
  }
}

void CIC::computeSineAssistanceTorque()
{
